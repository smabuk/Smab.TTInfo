...
foreach (Division division in divisions) {
	...
	foreach (Team team in division.Teams) {
		...
		try {
			...
			Console.WriteLine($"  {TT365Reader.FixPlayerName(newTeam.Captain),-20}");
		}
		foreach (Player player in newTeam.Players?.OrderByDescending(p => p.WinPercentage).ToList() ?? new List<Player>()) {
			Console.WriteLine($"         {TT365Reader.FixPlayerName(player.Name),-25} {player.Played,6} {(int)player.WinPercentage,3}%");
			if (searchName is not null && player.Name.ToLowerInvariant().Contains(searchName)) {
				...
				foreach (PlayerResult playerResult in p2.PlayerResults.Where(pr => pr.PlayerTeamName == team.Name).OrderBy(pr => pr.Date)) {
					...
					Console.WriteLine($"          {dateString, -6} {(playerResult.ResultReason.Any() ? "*" : ""),1}{playerResult.Result.FirstOrDefault(),1}  {playerResult.RankingDiffString,3}  {TT365Reader.FixPlayerName(playerResult.Opponent.Name),-24}   {playerResult.OpponentTeam,-30}  {playerResult.GameScore,3}  {playerResult.Scores}");
